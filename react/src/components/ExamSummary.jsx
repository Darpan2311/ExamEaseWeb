      ...
      <div id="questions-list">
        {summaryData.questions.map((q, index) => (
          <div key={index} id="question-card-id" className="question-card">
            ...
            <p id={`user-response-${index}`}>
              <strong>Your Answer:</strong> {q.userResponse || "Not Answered"}
            </p>
            <p id={`correct-response-${index}`}>
              <strong>Correct Answer:</strong> {q.correctResponse}
            </p>
            ...
            <p id={`marks-awarded-${index}`}>
              <strong>Marks:</strong> {q.marksAwarded}
            </p>
            <p id={`user-time-${index}`}>
              <strong>Your Time:</strong> {q.userTimeTaken.toFixed(2)} seconds
            </p>
            <p id={`topper-time-${index}`}>
              <strong>Topper's Time:</strong> {q.topperTimeTaken.toFixed(2)} seconds
            </p>
          </div>
        ))}
      </div>
